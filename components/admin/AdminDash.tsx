...
export default function AdminDash({ email }: { email: string }) {
  ...
  const shownPhotos = photoFilter === "all" ? photos : photos.filter((p) => p.category === photoFilter);
  ...
  async function deletePhoto(p: Photo) {
    ...
    if (p.storage_path.startsWith("http")) {
      ...
      if (relPath) {
        ...
        await supabase.storage.from("thumbs").remove([relPath.replace(/\.jpg$/, ".webp")]);
      }
    }
    reload();
  }
  ...
  return (
    <main className="flex min-h-[100svh] flex-col bg-[#0a0908] md:flex-row">
      {/* ============ SIDEBAR ============ */}
      <aside className="flex shrink-0 flex-row items-center justify-between border-b border-[var(--line)] bg-[#070606] px-5 py-4 md:min-h-[100svh] md:w-60 md:flex-col md:items-stretch md:justify-start md:border-b-0 md:border-r md:px-0 md:py-0">
        <div className="md:border-b md:border-[var(--line)] md:px-7 md:py-8">
          ...
          <div className="font-display mt-1 text-xl font-light italic leading-none">
            Alan Kugelmass
          </div>
        </div>
        ...
        <nav className="flex gap-1 md:mt-6 md:flex-col md:gap-0 md:px-4">
          {NAV.map((n) => (
            <button
              key={n.id}
              onClick={() => setTab(n.id)}
              className={`group flex items-center gap-3 px-4 py-2.5 text-left transition-all md:py-3 ${
                tab === n.id
            ...
            >
              <span
                className={`hidden w-4 text-center text-sm md:block ${tab === n.id ? "text-[var(--accent)]" : "opacity-50"}`}
              >
                {n.glyph}
              </span>
              <span className="text-[10px] uppercase tracking-wide2">{n.label}</span>
              {n.badge && (
        ...
        <div className="md:mt-auto md:border-t md:border-[var(--line)] md:px-7 md:py-6">
          ...
          <div className="mt-0 flex gap-4 md:mt-3">
            <a
              href="/"
              className="text-[9px] uppercase tracking-wide2 text-[var(--fg-dim)] transition-colors hover:text-[var(--accent)]"
            >
              Gallery
            </a>
            <button
            ...
            <div className="mb-6 flex flex-wrap items-center gap-2">
              {["all", "weddings", "hotels", "documentary", "prints"].map((c) => (
                <button
                  ...
                  className={`border px-4 py-2 text-[9px] uppercase tracking-wide2 transition-all ${
                    photoFilter === c
